...
.DESCRIPTION
This script will send to a specified recipient a list of movies added to Plex in the past x days (defined by variable).
...
param(
    ...
    #[string]$Token,
    [string]$Token = 'YourPlexTokenHere',

    #Required: OMDB API Key
    [string]$omdbapi = 'YourOMDBAPIKey',
    ...
    foreach ($show in $tvShows) {
        ...
        if (-not($ExcludeLib.Contains($section))){
             ...
             if ($omdbResponse.Response -eq "True") {
                ...
                $tvList += "<td><li><a href=`"http://www.imdb.com/title/$($omdbResponse.imdbID)/`">$($tvshowname)</a></li>"
                ...
                $tvList += "<tr><td><img src=`"$imgPlex`" height=150px width=150px></td><td><li>$($tvshowname)</a></li>"
                            $tvList += "<td><li><a href=`"http://www.imdb.com/title/$($omdbResponse.imdbID)/`">$($tvshowname)</a></li>"
